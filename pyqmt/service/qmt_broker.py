...
发布态 live 能力应通过 qmt-gateway 获取，而不是由主体直接加载 xtquant。
仅当明确开启兼容开关时，才允许继续使用本地 QMT broker。
...
def _ensure_legacy_local_qmt_enabled() -> None:
    """确保显式开启本地 QMT 兼容路径。"""
    if os.getenv(LEGACY_LOCAL_QMT_ENV, "").strip() == "1":
        return
    raise RuntimeError(
        "QMTBroker 已退役为兼容路径，默认不可用。"
        "请改用 qmt-gateway；若确需临时使用本地 QMT，请显式设置 "
...
class QMTBroker(AbstractBroker):
    def __init__(self, account_id: str, portfolio_id: str = "qmt"):
        _ensure_legacy_local_qmt_enabled()
