...
from pyqmt.dal.tradedb import db, new_uuid_id
...
def as_order(xt_order: XtOrder) -> OrderModel:
    ...
    db_order = db.get_order_by_fid(xt_order.order_id)
    if db_order is None:
        logger.warning(
...
def as_xt_order_params(order: OrderModel, acc: StockAccount) -> dict:
...
def as_trade(xt_trade: XtTrade) -> TradeModel:
    ...
    db_order = db.get_order_by_fid(xt_trade.order_id)
    if db_order is None:
        logger.warning(
...
class MyXtQuantTraderCallback(XtQuantTraderCallback):
    ...
    def on_order_error(self, order_error: XtOrderError):
        ...
        if qtoid is None or not qtoid.startwith("qtoid-"):
            # 完全的废单，无法关联。
            logger.warning(
...
class QMTBroker(AbstractBroker):
    def __init__(self, cfg):
        self.account_id = cfg.qmt.account_id
    ...
    async def query_orders(self, cancelable_only: bool = False) -> pl.DataFrame:
