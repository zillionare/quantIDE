...
from pyqmt.core import str2date
...
class Chores:
    ...
    def get_bars_cache_status(self, frame_type: FrameType)->Tuple[datetime.date, datetime.date] | None:
        """获取已缓存的行情数据状态
        
        Args:
            frame_type: 周期类型
        """
        sql = "select start, end from bars_cache_status where frame_type = ?"
    ...
    def calc_bars_cache_start(self, frame_type: FrameType)->datetime.date:
        ...
        Args:
            frame_type: 行情周期
        """
        sql = f"select * from sys where key = max_{frame_type.value}_bars_on_init"
    ...
    def save_bars_cache_status(self, start: datetime.date|str, end: datetime.date|str, frame_type: FrameType):
        with self.con.cursor() as cursor:
            ...
            if result is None:
                ...
                cursor.execute(sql, (start, end, frame_type.value))
                return
