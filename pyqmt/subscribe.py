...
def sync_1m_bars(codes: List[str]):
    start_ = "20240312"
    xtd.download_history_data2(codes, period='1m',start_time=start_, end_time='',callback=lambda x: x)

    print(f"{os.getpid()} get result...")
    barss = xtd.get_market_data_ex([], codes, '1m', start_time='', count=241, dividend_type='front', fill_data=True)
    # print(barss)
