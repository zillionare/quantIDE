...
def test_bills(setup_db):
    ...
    # 1. 准备当前组合的数据
    db.insert_order(Order(portfolio_id, "000001.SZ", OrderSide.BUY, 100, BidType.MARKET, tm=tm, qtoid="o1"))
    db.insert_trades(Trade(portfolio_id, "t1", "o1", "f1", "000001.SZ", 100, 10.0, 1000.0, tm, OrderSide.BUY, "c1"))
    db.upsert_positions(Position(portfolio_id, dt, "000001.SZ", 100, 100, 10.0, 0.0, 1000.0))
    db.upsert_asset(Asset(portfolio_id, dt, 1000000.0, 1000000.0, 0.0, 0.0, 1000000.0))

    # 2. 准备另一个组合的数据以验证过滤
    other_p = "other_p"
    db.insert_order(Order(other_p, "000002.SZ", OrderSide.BUY, 200, BidType.MARKET, tm=tm, qtoid="o2"))
    db.insert_trades(Trade(other_p, "t2", "o2", "f2", "000002.SZ", 200, 20.0, 4000.0, tm, OrderSide.BUY, "c2"))
...
def test_metrics_insufficient_benchmark(setup_db):
    ...
    # 策略日期 1-5
    for i in range(5):
        db.upsert_asset(Asset(portfolio_id, datetime.date(2024, 1, i + 1), 100.0, 100.0, 0.0, 0.0, 100.0))
...
def test_metrics_empty_data(setup_db):
    # 数据不足（少于 2 天无法计算收益率）
    db.upsert_asset(Asset("empty", datetime.date(2024, 1, 1), 100.0, 100.0, 0.0, 0.0, 100.0))
