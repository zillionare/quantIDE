brokers 由三类 broker 组成，回测、仿真和实盘，以实现各种场景下的委托（buy/sell）、撮合（自己撮合或者交给第三方撮合，比如在实盘情况下），以及账户管理（包括在账户管理之上衍生出来的策略评估）等功能。
...
使用 quantstats 进行策略评估。从 assets 表中，按 portfolio_id 获取该组合的每日资产，再计算出每日收益，即可通过 quantstats 评估该组合。
...
broker 进行撮合、以及在不指定委托量进行委托时，需要两个关键的数据，bid_time 发生之后的当天价格，以及当天的涨跌停价。
...
一个委托指令通常需要提供委托的资产、数量、价格等信息。为策略评估需要，每个委托指令都需要带有 portfolio_id。
...
回测撮合的实现依赖于 bid_time 以及所使用的数据源。

系统没有显式的开盘买入规则，一切由 bid_time 来决定，以便同时适应日线、分钟线撮合。即，如果策略希望是以开盘价买入，则将 bid_time 设置为早于（含等于）开盘时间；如果 bid_time 为当日盘中交易时间，则在日线行情下，将以**收盘价**成交；在分钟线行情（v0.1 暂不实现）下，则从 bid_time 起，按每分钟收盘价、成交量进行匹配，直到当天结束或者订单完成。如果在 bid_time 之后，遇到涨、跌停，则涨跌停期间的成交量不参与匹配。

如果数据源是日线，则撮合时不看成交量，只要价格能够匹配，全部允许成交。

在数据源为日线时，如果遇到涨跌停，则不允许交易。比如，如果策略要求的开盘价买入（即将 bid_time 设置为小于开盘时间），而开盘即涨停，则不允许交易；但如果开盘涨停，收盘打开涨停，且 bid_time 为盘中时间，则允许以收盘价买入。对跌停亦同样处理。
...
2. abstract_broker.py, 抽象类。实现超时控制、策略评估功能。
