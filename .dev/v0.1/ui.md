...
当设备尺寸大于等于1200px时，上述内容区宽度固定为1200px，居中显示。宽度介于1000px 和1200px 之间时，使用流式布局，调整模块间距、字体大小、图表尺寸以适应。当设备尺寸小于1000px时，header 仅显示 logo， 仿真交易|实盘交易 和用户 profile，点击仿真、实盘则分别显示交易界面，其它功能隐藏。

以下设计如无明确说明，均指电脑端。

header占据60px的高度，其余区域占满剩余高度。顶层页面绝对禁止出现横向溢出，一般不使用纵向溢出。
...
以上功能分为三组。logo 与 brand是一组，右对齐，元素之间合理间隔。第二组空白区域，自由伸展，用于视觉平衡。第三组为一级导航菜单、用户头像。右对齐。组内元素之间合理间隔。
...
Sidebar 占据左侧约200px 宽度。其它区域都占满剩余宽度。

## 5. 功能模块详细设计

### 5.1. 用户档案与登录

见 [登录与退出](v0.1/ui/login.md)
...
详细的数据表结构请参考 [brokers.md](v0.1/brokers.md) 文档。
