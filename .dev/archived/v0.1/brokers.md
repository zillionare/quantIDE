...
与 portfolio 关联的主要状态是 Asset 和 Position。在 backtest portfolio 中，所有的状态更新会立即 写入到数据库；在 simulation 与实盘 portfolio 中，状态更新比较频繁（每秒数次），并且这些状态并不是最终状态，所以会保存在内存中（通过 broker 实例）以供查询，但在每日结束时，会写入数据库。

生成的 Order 与 Trade，会在生成和状态更新时，及时写入数据库。

### 初始化

### 收盘

仿真 broker 可能需要在收盘时，清理所有未完成的订单。
