# Init Wizard

在系统安装后首次启动时运行本初始化向导。亦可通过 /init-wizard?force=true 强制重新运行。

!!! attention
    强制重新运行一般仅用于测试。在生产环境下禁止使用。这可能导致之前的数据丢失。

## Scope

init-wizard 的作用是：

- 配置运行时环境，比如行情数据存放目录。应用配置数据库固定保存在系统配置目录。
- 配置管理员密码（账号锁定为 admin）
- 配置本服务的监听端口，以及是否仅限本机使用
- 配置实时行情与交易网关（如 qmt-gateway）
- 配置通知渠道
- 配置数据源（tushare token），epoch 及初始下载数据大小，并下载历史数据。
- 在完成后，应该进入/ (但会被拦截到/auth/login)

## 通用 UI 说明

![](https://cdn.jsdelivr.net/gh/zillionare/imgbed2@main/images/2026/03/20260329160311.png)

UI将使用 monster ui中的 steps 组件来创建，布局如上图所示。

左侧1区为向导步骤，右侧2区为配置项容器，二者等高。2区内部再明确拆为三个区域：

1. a 区：标题与说明区。title 内容来自于向导流程中的列表，但具体步骤中可能特别说明，以具体步骤文档为准。校验错误信息也显示在这里，位于说明之后。
2. b 区：表单区。用户在这里填写配置项。该区域在 2 区剩余空间内垂直居中显示。
3. c 区：操作区。这里放置“上一步 / 下一步 / 进入系统”等按钮，并要求与 1 区底部对齐。

布局约束：

1. a 区与 1 区顶端对齐。
2. c 区与 1 区底部对齐。
3. b 区在 2 区中居中显示。
4. step 组件本身只负责渲染 b 区内容，不再各自重复封装 title、description 和按钮区。

## 进入规则

### 第一次访问时

在应用初始化之前， 所有请求，包括/init-wizard 都应该被重定向到 `/init-wizard`。

### 初始化完成后再次访问

Once initialization has completed, revisiting `/init-wizard` redirects back to `/`.

一旦始化完成后再次访问/init-wizard，应该被重定向到 / ，除非带有 `?force=true`参数。该参数应该只被用于测试。再次初始化时，可能之前的数据丢失（比如选择了不一样的数据目录）。

## 向导流程

向导共有6步。

1. 欢迎
2. 运行环境配置
3. 设置管理员密码
4. 配置交易/实时行情网关
5. 数据源设置及下载
6. 完成

向导会将每步状态保存在 app_state 中，最终写入数据库。

### Step : 欢迎步

title: 欢迎使用 Quant IDE!

introduce: QuantIDE 是为量化人打造的集成开发环境 -- 数据、研究、回测、实盘。

本向导将引导您完成以下工作：

1. 配置运行时环境，比如数据存放目录。
2. 配置管理员密码
3. 配置交易/实时行情网关
4. 配置通知渠道
5. 配置数据源并下载历史数据。

### Step 2: 运行环境

这一步配置以下选项：

| 可选？ | 配置项         | 控件类型   | placeholder | 变量名和类型         | default     | tip                                                                                 | 说明                                            |
| ------ | -------------- | ---------- | ----------- | -------------------- | ----------- | ----------------------------------------------------------------------------------- | ----------------------------------------------- |
| Y      | 数据存储位置   | 文本输入框 | ~/.quantide | app_home, str        | ~/.quantide | 行情数据和相关缓存将存放在此处。留空时使用 ~/.quantide，并在保存时执行 expanduser。 | 配置数据库固定保存在系统配置目录，不在这里存放  |
| Y      | 只允许本机访问 | 复选框     | 无          | localhost_only, bool | true        | 是否仅允许本机访问                                                                  | 默认勾选。如未勾选，则 app_host 取值为'0.0.0.0' |
| Y      | 监听端口       | 文本输入框 | 8130        | app_port, int        | 8130        | 可选。除非端口已被其它应用占用，否则可使用默认值                                    | NA                                              |
| Y      | 路径前缘       | 文本输入框 | /quantide   | app_prefix, str      | '/quantide' | 可选。如果不明白含义，可保持默认                                                    | NA                                              |

在实现时，要注意说明中的检查项。如果校验不通过，则进行错误提示，不得进入下一步。数据存储位置不是必填项，用户留空时应回退到默认值 ~/.quantide，并在写入状态前展开为实际用户目录。

### Step 3: Administrator Password

设置管理员密码。两次输入，必须相同才能允许进入下一步。

提示用户使用8位以上密码，字母大小写、数字、特殊符号各一。用户输入第一个框完成后，即给出密码强弱评估，如果是强密码，则给出绿色提示；弱密码给出红色提示；中等给予黄色提示。但只要两次输入一致，允许用户进入下一步。风险由用户自己承担。

### Step 4: Gateway

由一个复选框加上若干文本输入框组成。

1. 复选框，是否启用 gateway。tooltip 信息：请安装 quantide gateway并配置，否则无法获得实时行情和执行交易。默认勾选，如果用户取消，则余下的输入框都 disable掉。
2. 必填（如果勾选了复选框）gateway 服务器地址，默认值为 localhost。
3. 必填（如果勾选了复选框）gateway 端口，默认值为 8000。
4 必填（如果勾选了复选框） gateway 访问密钥，默认值为空字符串。提示信息，可在 gateway 用户头像菜单中生成和查看密钥。
5. 可选填，路径前缘，默认值为 /。

校验：点击下一步时，进行校验。如果校验不通过，可以提示用户先不勾选复选框，后面再配置。校验逻辑是调用 http://gateway:port/prefix/ping
如果能返回200，则配置正确。否则，提示用户，要么重新配置，要去暂时不勾选复选框。

### 初始化后的入口约束

首次初始化完成后，系统不应再以“先创建交易账号”为默认引导。

1. 首次初始化的关键前置项是 gateway 配置。用户可以在 Step 4 中完成配置，也可以显式选择暂不启用。
2. 当用户未启用 gateway 时，登录后首页不弹出“创建模拟账户/创建实盘账户”对话框。
3. 当用户未启用 gateway 时，系统应仍允许用户进入“策略”和“系统维护”相关页面，继续完成数据维护、策略查看与后续配置。
4. 当用户未启用 gateway 且点击“实盘”或“仿真”入口时，应提示用户先完成 gateway 配置，并提供跳转到 `/system/gateway/` 或 `/init-wizard?force=true` 的入口。
5. 该提示应在页面导航层或入口层完成，不应把用户直接带到“创建账户”流程。


### Step 5: Data Initialization And Download

描述是： 配置数据源，触发首次下载。首次下载可以仅下载少量数据，后续系统会以后台任务继续下载，直到数据补齐到您设定的数据起始日。

这一步将配置以下选项。

- 数据起始日。保存变量为 epoch, 类型为 datetime.date，默认为2005年1月1日。 placeholder 为 2005-01-01。tooltip 为：行情数据的起始日，为确保数据有效、一致，不建议配置太早的起始日。比如，tushare 的数据集中，ST/涨跌停历史数据可能会从2016年起。
- Tushare 访问密钥。tooltip 为，访问 tushare 需要密钥，请在 https://tushare.pro/user/token 页面获取。
- 首次下载时长。单位为年，默认为1年。tooltip 为：本次初始化时，会下载从今天起往前推若干年的数据，默认为1年。后续还会有后台任务继续下载，所以为使您开速进入系统使用，建议就设置为1年。下载一年的数据，大约需要30分钟左右，也取决于您账号的限速。

在首次下载时长设置下面，有一段描述，介绍在当前选择下，数据下载的起止时间，当用户更改设置时自动更新。计算时使用自然日历。

在上述描述之下，还有一段描述，介绍将下载的数据种类，即：

- 证券日历
- 全A 证券列表
- 历史日线行情（含复权因子与涨跌停价格）
- ST 数据

在用户点击下一步时，根据用户设置，进行数据下载，并在下载的过程中，报告下载进度。下载进度将通过下面的对话框展示。

![](https://cdn.jsdelivr.net/gh/zillionare/imgbed2@main/images/2026/03/20260329164234.png)

当下载完成后，对话框消失，自动进入到下一步。

如果下载中出现错误，需要在进度对话框中显示错误信息，并在进度对话框消失（比如进度完成，但有错误），在 wizard 对话框中显示错误信息。

**变更**： 这一步有一个下载按钮，取消。功能由『下一步』代替。

### Step 6: Complete

描述信息：恭喜！您的系统已经初始化完成。点击下方按钮，立即进入系统。

这一步按钮显示在 c 区，为“进入系统”。

## 其它

现 init-wizard 中，有一步是设置通知的，这一步暂不实现，代码先删除。

## 再初始化

如果用户通过 init-wizard?force=true 强制再次初始化时，需要代入已经保存的选项。如果用户选择的数据目录不变，则不重新初始化数据库表。
