...
3. 日线行情还包含`factor`(复权因子)、`is_st`(是否 st, bool)，`up_limit`(涨停价)和`down_limit`(跌停价)。除`is_st`外，其它都是64位浮点数。
4. 一条完整日线行情记录必须同时包含 asset, frame, open, hight, low, close, volume, amount, factor, is_st, up_limit 和 down_limit，并且已转换为规定的格式。
